...
<%-- <script src="${path}/resources/js/bid_List.js"></script>
 --%>
</head>
<body>
				...
				<table class="table table-inbox table-responsive table-bordered">
					...
					<tbody>
				 	...
				 	<c:forEach items="${auctioninfoList}" var="auction">
							    <!-- 프라퍼티이름 변경 -->
							    <c:choose>
							    	<c:when test="${not auction.a_CONDITION eq '검토전'}">
							    <tr>
							        <td class="inbox-small-cells">
							            <input type="checkbox" class="mail-checkbox" data-auction-num="${auction.a_NUM}">
							        </td>
							        <td align="left" class="auction-id">${auction.a_NUM}</td>
							        <td align="left"><a href="bid_Ck_Pc.do?A_NUM=${auction.a_NUM}">${auction.a_TITLE}</a></td>
							        <td>${auction.a_DATE}</td>
							        <td>${auction.a_CONDITION}</td>
							        <!-- 추가 -->
							       <td align="left">
									<a href="bid_Witmodify.do?A_NUM=${auction.a_NUM}"><input type="button" value="수정"/></a><!-- 수정페이지로 날릴것. -->
									<a href="bid_Delete.do?A_NUM=${auction.a_NUM}"><input type="button" value="삭제"/></a>
									</td>
							    </tr>
							     	</c:when>
							            <c:otherwise>
			...
			<div class="card-body">
				<table class="table table-inbox table-responsive table-bordered">
					...
					<tbody>
				 	...
				 	<c:forEach items="${auctioninfoList}" var="auction">
							    <!-- 프라퍼티이름 변경 -->
							    <c:choose>
							    	<c:when test="${auction.a_CONDITION eq '검토전'}">
							    <tr>
							        <td class="inbox-small-cells">
							            <input type="checkbox" class="mail-checkbox" data-auction-num="${auction.a_NUM}">
							        </td>
				...
				<div class="card-body">
					<div class="row row-cols-4">
						<div class="col-2"></div>
						<a class="btn btn-primary col-4" href="bid_Wit.do">새로운 경매</a> 
						<a id="deleteButton" class="btn btn-primary col-4 " href="#">선택 항목 삭제</a>
					</div>
				</div>
			</div>
			</div>
